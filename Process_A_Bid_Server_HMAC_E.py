...
class MyTCPHandler(socketserver.BaseRequestHandler):
    def handle(self):
        ...
        Encrypted_msg = self.data[:len(self.data)-64]
        Decrypted_msg = str(Encryption.cipher.decrypt(Encrypted_msg))

        secret = b'1234'
        real_tag = hmac.new(secret, bytes(Decrypted_msg, 'utf-8'), digestmod=hashlib.sha3_512).digest()

        tag = self.data[-64:]
        # validates the authenticity of the message
        if (tag == real_tag):
            # separates the attributes sent in by message and displays them
            attributes = (Decrypted_msg).split()
